...
import org.tensorflow.data.NdArrayDataset;
...
public class NioMNIST {
    ...
    public static void main(String[] args) throws IOException, NoSuchAlgorithmException {
        Tuple2<NdArrayDataset<Float>> nio =  NioMNIST.nioTensorFrames();
    }

    public static Tuple2<NdArrayDataset<Float>> nioTensorFrames() throws IOException, NoSuchAlgorithmException {
        // Download MNIST files if they don't exist.
        NioMNIST.download();

        // Read data files into arrays
        FloatNdArray trainImages = readImagesToFloatBuffer(Keras.path(LOCAL_PREFIX, TRAIN_IMAGES));
        FloatNdArray trainLabels = readLabelsToFloatBuffer(Keras.path(LOCAL_PREFIX, TRAIN_LABELS));
        FloatNdArray testImages = readImagesToFloatBuffer(Keras.path(LOCAL_PREFIX, TEST_IMAGES));
        FloatNdArray testLabels = readLabelsToFloatBuffer(Keras.path(LOCAL_PREFIX + TEST_LABELS));

        // Create tensor frame
        NdArrayDataset<Float> train = new NdArrayDataset<>(trainImages, trainLabels);
        NdArrayDataset<Float> test = new NdArrayDataset<>(testImages, testLabels);
