...
//        float[][] trainImages = MNIST.readImages(Keras.path(LOCAL_PREFIX, TRAIN_IMAGES).toString());
//        float[][] trainLabels = MNIST.readLabelsOneHot(Keras.path(LOCAL_PREFIX, TRAIN_LABELS).toString());
//        float[][] testImages = MNIST.readImages(Keras.path(LOCAL_PREFIX, TEST_IMAGES).toString());
//        float[][] testLabels = MNIST.readLabelsOneHot(Keras.path(LOCAL_PREFIX + TEST_LABELS).toString());
//
...
//
//        // Read data files into arrays
//        float[][][] trainImages = MNIST.readImages2D(Keras.path(LOCAL_PREFIX, TRAIN_IMAGES).toString());
//        float[][] trainLabels = MNIST.readLabelsOneHot(Keras.path(LOCAL_PREFIX, TRAIN_LABELS).toString());
//        float[][][] testImages = MNIST.readImages2D(Keras.path(LOCAL_PREFIX, TEST_IMAGES).toString());
//        float[][] testLabels = MNIST.readLabelsOneHot(Keras.path(LOCAL_PREFIX + TEST_LABELS).toString());
//
//        // Return a pair of graph loaders; train and test sets
//        return new Pair<>(
